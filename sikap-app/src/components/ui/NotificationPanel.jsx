...
export default function NotificationPanel() {
  ...
  const [readIds, setReadIds] = useState(() => {
    try { return JSON.parse(localStorage.getItem('sikap_read_notifs') || '[]') }
    catch { return [] }
  })
  ...
  function markAllRead() {
    ...
    setReadIds(allIds)
    localStorage.setItem('sikap_read_notifs', JSON.stringify(allIds))
  }

  function markRead(id) {
    const updated = [...new Set([...readIds, id])]
    setReadIds(updated)
    localStorage.setItem('sikap_read_notifs', JSON.stringify(updated))
  }
